...
// when this StringCounter is being used as a string pool for another map, remove operation in enclosing HashMap
...
[[maybe_unused]] static void sct_policy_value_free_stringpool(StringCounter ismap[static 1], const MapNode node[static 1]) {
    if ( node->value.value_type == MAP_TYPE_STRING) {
        sct_remove(ismap, node->value.vstring);
    }
}
...
static void sct_free_key_if(StringCounter ismap[static 1], const MapNode node[static 1]) {
    ...
    if ( map->policies.key_policies.on_free_key ) {
        ...
        sct_policy_key_free_default(map, node->key);
    }
}

static void sct_free_value_if(StringCounter ismap[static 1], const MapNode node[static 1]) {
    if (!ismap || !ismap->map || !node) return;
    HashMap *map = ismap->map;
    if (map->policies.value_policies.on_free_value) {
        ...
        sct_policy_value_free_default(map, node->value);
    }
}

static void sct_destroy_node(StringCounter ismap[static 1], MapNode *node) {
    if (!ismap) return;
    sct_free_key_if(ismap, node);
    sct_free_value_if(ismap, node);
    free(node);
}
...
StringCounter * sct_create(const size_t num_buckets) {
    ...
    StringCounter *ismap = malloc(sizeof(StringCounter));
    if (!ismap) {
        return nullptr;
    }
    ...
    ismap->map->policies.key_policies   = DEFAULT_sct_KEY_POLICIES;
    ismap->map->policies.value_policies = DEFAULT_sct_VALUE_POLICIES;

    return ismap;
}

void sct_destroy(StringCounter ismap[static 1]) {
    if (!ismap->map)  return;
    
    HashMap *map = ismap->map;
    // todo in sct_create, we just allocate the StringCounter wrapper, then delegate to map_create().
    // see if we can refactor this sct_destroy() method to work similarly. The only difference is that if the
    ...
    // parent HashMap.
    sct_clear(ismap); // this frees all allocated MapNodes, keys, and values
...
void sct_clear(StringCounter ismap[static 1]) {
    if (!ismap->map) {
        return;
    }
    HashMap *map = ismap->map;
...
bool sct_contains_key(StringCounter ismap[static 1], const char* strkey) {
...
long sct_get_count(const StringCounter ismap[static 1], const char *key) {
    if (!ismap->map) {
        return 0;
    }
...
const char* sct_ref(StringCounter ismap[static 1], char string[static 1]) {
...
void sct_unref(StringCounter ismap[static 1], const char *strkey) {
    if (!ismap->map) return;
...
bool sct_is_empty(StringCounter ismap[static 1]) {
    return map_is_empty(ismap->map);
}

void sct_put(StringCounter ismap[static 1], const char* strkey, long value) {
    if ( !ismap->map ) return;
...
void (sct_remove)(StringCounter ismap[static 1], const char* strkey) {
    if (!ismap->map) return;
...
size_t sct_size(StringCounter ismap[static 1]) {
...
void sct_repr_InternStringMap(StringCounter ismap[static 1], const bool verbose) {
    if (!ismap) {
        printf("(StringCounter)nullptr");
        return;
    }
    map_repr_HashMap(ismap->map, verbose, "StringCounter");
}